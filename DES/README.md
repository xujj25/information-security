...
![总体结构](C:\Users\XUJIJUN\Desktop\Grade_three_1st\信息安全技术\许基骏作业\HW1\总体结构.png)
...
1. 密钥调度模块
   - 子密钥生成模块
      1. PC-1置换模块（将输入的64位带校验位的密码块按照PC-1置换表的规定置换成56位的块）
      2. 循环左移模块（将1中生成的56位块的前后28位分别通过16次循环左移指定位数生成16个子块）
      3. PC-2压缩置换模块（将2中生成的子块按照PC-2置换表的规定压缩置换成16个48位的子密钥）
2. $IP$ 置换模块（将输入的明文块按照IP置换表的规定进行置换）
3. 16 轮迭代 $T$ 模块（将IP置换的结果分成左右两半进行16轮迭代）
   - 迭代逻辑模块：$L_i=R_{i-1},R_i=L_{i-1}\oplus{f(R_{i-1},K_i)}$
   - *Feistel* 轮函数 $f(R{_{i-1}, K_i})$ 
     1. E-扩展模块（将长度为32位的串$R_{i-1}$ 作E-扩展，成为48位的$E(R_{i-1})$）
     2. 将$E(R_{i-1})$ 和长度为48位的子密钥$K_i$作48位二进制串按位异或，$K_i$ 由密钥$K$ 生成
     3. S-盒模块（将2得到的结果平均分成8个分组，分别经过8个不同的S-盒进行6-4转换，得到8个长度分别为4位的分组）
     4. 将3得到的分组合并得到长度为32位的串
     5. P-置换模块（将4的结果根据P-置换表的规定进行置换得到*Feistel* 轮函数的最终结果）
4. $IP^{-1}$ 置换模块（将输入的明文块按照$IP^{-1}$置换表的规定进行置换）
...
以下是我使用C++的DES算法实现，已经经过单块加密测试。完整项目可以访问本人[github](https://github.com/xujj25/information-security/tree/master/DES) 。
